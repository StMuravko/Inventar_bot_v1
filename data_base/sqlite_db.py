...
def sql_start():
    ...
    base.execute('CREATE TABLE IF NOT EXISTS inventar(photo TEXT, name TEXT PRIMARY KEY, groups TEXT, quantity INTEGER, loaded_by TEXT)')
    base.commit()


async def sql_add_comand(state):
    async with state.proxy() as data:
        cur.execute(f'INSERT INTO inventar VALUES (?,?,?,?,?) ON CONFLICT DO UPDATE SET quantity = quantity + {data["quantity"]};', tuple(data.values()))
        base.commit()
...
async def sql_delete_command(state):
    async with state.proxy() as data:
        cur.execute(f'UPDATE inventar SET quantity = quantity - {data["quantity"]} where name = "{data["name"]}"')
        base.commit()